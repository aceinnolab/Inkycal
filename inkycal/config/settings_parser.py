...
class Settings:
  ...
  """
    ...
    def __init__(self, settings_file_path: str) -> None:
        ...
        if len(all_heights) == num_modules:
            ...
            # if all heights are specified, use given values
                ...
                top_height = [self.__to_decimal(_['height']) for _ in
                              self._settings['panels'] if _['location'] == 'top']

                middle_height = [self.__to_decimal(_['height']) for _ in
                                 self._settings['panels'] if _['location'] == 'middle']

                bottom_height = [self.__to_decimal(_['height']) for _ in
    ...
    @staticmethod
    def __to_decimal(percentage: float) -> float:
        """
        Convert a given percentage to a decimal number
        """
    ...
    def __validate(self) -> None:
        ...
        for param in required:
            if param not in settings:
                raise Exception('required parameter: {} not found in settings file!'.format(param))
        ...
        if not isinstance(self.language, str) or self.language not in self._supported_languages:
            print('Language not supported, switching to fallback, en')
            self.language = 'en'

        if not isinstance(self.units, str) or self.units not in self._supported_units:
            print('units not supported, switching to fallback, metric')
            self.units = 'metric'

        if not isinstance(self.hours, int) or self.hours not in self._supported_hours:
            print('hour-format not supported, switching to fallback, 24')
            self.hours = 24

        if not isinstance(self.model, str) or self.model not in self._supported_models:
            print('model not supported, switching to fallback, epd_7_in_5')
            self.model = 'epd_7_in_5'

        if not isinstance(self.update_interval, int) or self.update_interval not in self._supported_update_interval:
    ...
    def active_modules(self) -> list:
        return [section['type'] for section in self._settings['panels']]

    def common_config(self) -> dict:
        return {'language': self.language, 'units': self.units, 'hours': self.hours}
    ...
    def get_config(self, module_name: str) -> dict:
        ...
        # TODO: fix error: size and config can be referenced before assignment
        return {'size': size, 'config': {**config, **self.common_config()}}

    def get_position(self, module_name: str) -> Union[None, str]:
        """
