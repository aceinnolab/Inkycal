...
class Calendar(inkycal_module):
    ...
    def generate_image(self) -> None:
        ...
        if self.show_events:
            ...
            calendar_height = self.height - month_name_height - weekdays_height
            logger.debug('calendar-section size: {0} x {1} px'.format(im_width, calendar_height))
        ...
        weekday_pos = [(grid_start_x + icon_width * _, month_name_height) for _ in range(calendar_cols)]
        ...
        for i in range(len(weekday_pos)):
            write(
                im_black,
                weekday_pos[i],
        ...
        if self.show_events:
            ...
            line_spacing = 0
            # TODO: fix error: events_height might be referenced before assignment
            ...
            del upcoming_events[:max_event_lines]
            ...
            if upcoming_events:
                ...
                # TODO: following two variables are never used?
                ...
                for event in upcoming_events:
                    ...
                    _time = event['begin'].format(self.time_format, locale=lang)
                    ...
                    if now < event['end']:
                        ...
                        if parser.all_day(event):
                            write(im_black, (date_width, event_lines[cursor][1]),
                                  (event_width_l, line_height), _name, font=self.font,
                                  alignment='left')
                        else:
                            write(im_black, (date_width, event_lines[cursor][1]),
                                  (time_width, line_height), _time, font=self.font,
                                  alignment='left')

                            write(im_black, (date_width + time_width, event_lines[cursor][1]),
                                  (event_width_s, line_height), _name, font=self.font,
                                  alignment='left')
                        cursor += 1
            else:
                symbol = '- '
