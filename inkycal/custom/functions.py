...
def print_fonts() -> None:
    ...
    """
    for _fonts in available_fonts:
        print(_fonts)


def get_system_tz() -> Union[str, None]:
    """
...
def auto_fontsize(font: ImageFont, max_height: float) -> ImageFont:
    """
...
def write(image: Image.Image, xy: tuple, box_size: tuple, text: str, font: ImageFont = None, **kwargs) -> None:
    """
    ...
    if (autofit is True) or (fill_width != 1.0) or (fill_height != 0.8):
        size = 8
    ...
    # Uncomment following two lines and comment out above two lines to show
...
def text_wrap(text: str, font: ImageFont = None, max_width: float = None) -> list:
    """
...
def draw_border(image: Image.Image, xy: tuple, size: tuple, radius: int = 5, thickness: int = 1,
                shrinkage: tuple = (0.1, 0.1)) -> None:
    """
